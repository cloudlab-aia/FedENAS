...
class MacroChild(Child):
  def __init__(self,
    ...
    if self.transfer:
      self.weights.weight_map = child_weights
      ...
      with fw.name_scope(child.name):
        ...
        with fw.name_scope("fc") as scope:
          self.model_dropout = MacroChild.Dropout(
            ...
            scope,
            layers_channels[-1])
        # print(f"CHILD TRAINABLE VARIABLES: \n {child.trainable_variables()}")
        # print(f"CHILD WEIGHTS: \n {child.weights.weight_map}")
  ...
  def _build_train(self):
    ...
    self.train_model = MacroChild.Model(self, True, reuse=self.transfer)
  ...
  def _build_valid(self):
      ...
      #   arch_1 = pickle.load(f)

      # for layer in arch_1["child_valid_model_weights"]:
      #     self.valid_model.child.weights.weight_map[layer] = fw.Variable(arch_1["child_valid_model_weights"][layer].value(),name=layer,trainable=True)
  ...
  def _build_test(self):
      ...
      # for layer in arch_1["child_test_model_weights"]:
      #     self.valid_model.child.weights.weight_map[layer] = fw.Variable(arch_1["child_test_model_weights"][layer].value(),name=layer,trainable=True)
